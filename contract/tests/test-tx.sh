...
ESTATE_NODE=tcp://localhost:26457
CO_CHAIN=ibc0
TRAIN_CHAIN=ibc1
HOTEL_CHAIN=ibc2
...
${NODE_CLI} query --home ${CO_HOME} contract call --node ${DCC_NODE} --from ${ACC0} --keyring-backend=test dcc transfer 0x00000001 --chain-id ${TRAIN_CHAIN} --save ./data/dcc.json
${NODE_CLI} query --home ${CO_HOME} contract call --node ${ESTATE_NODE} --from ${ACC0} --keyring-backend=test estate transfer 0x00000002 --chain-id ${HOTEL_CHAIN} --save ./data/estate.json
...
TX_ID=$(${NODE_CLI} tx --home ./data/ibc0/n0/simappcli cross create --from ${ACC0} --keyring-backend=test --chain-id ${CO_CHAIN} --yes \
    ...
    --contract ./data/estate.json --channel ${SRC02_CHAN}:${SRC02_PORT} \
